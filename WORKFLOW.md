...
For Linear GraphQL reads, do not use legacy query shapes such as `issueV2` or `issue(identifier: ...)` because they fail against the current API and waste rate-limit budget. Prefer:
- `issues(filter: { id: { in: ["<issue-id>"] } }, first: 1) { nodes { ... } }` when you know the internal issue id
- `issues(filter: { project: { slugId: { eq: "<project-slug>" } }, state: { name: { in: ["Todo"] } } }, first: N) { nodes { ... } }` for project-scoped searches
- narrow field selections only; never fetch broad issue payloads unless they are required
...
- If a non-GitHub required tool is missing, or required non-GitHub auth is unavailable after the branch is pushed and the PR is already attached, you may move the ticket to `Human Review` with a short blocker brief in the workpad that includes:
  - what is missing,
  - why it blocks required acceptance/validation,
  - exact human action needed to unblock.
- If no PR is attached yet, do not move the ticket to `Human Review`. Keep it in an active state, record the blocker brief in the workpad, and rely on workspace hooks / retry setup to restore the environment.
...
12. Only then move issue to `Human Review`.
    - Exception: if blocked by missing required non-GitHub tools/auth per the blocked-access escape hatch, move to `Human Review` only when a PR is already attached; otherwise keep the issue active with the blocker brief in the workpad.
